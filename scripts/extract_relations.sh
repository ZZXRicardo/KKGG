...
#
# 用法:
#   ./extract_relations.sh <input_dir> <output_dir> <prompt_file> [options...]
#
# 必填（3个）:
#   input_dir      : 输入 JSONL 目录（通常是实体提取输出）
#   output_dir     : 输出目录
...
#
# 示例:
#   ./extract_relations.sh out/entities/ out/relations/ prompts/relation.txt qianwen 0.5 0 -1 resume ./logs/rel_run1
# ===========================================
...
if [ "$#" -lt 3 ]; then
    ...
    echo ""
    echo "参数说明:"
    echo "  input_dir   : 输入 JSONL 目录（必须存在）"
...
# 错误条目自动保存至 $OUTPUT_DIR/errors（由 Python 逻辑处理）
